...
age_prop = pd.read_csv("prop_table.csv", index_col="time").iloc[:, 1:]
...
for age in df_source["Real age"].unique():
    ...
    natural_survival = df_source[df_source["Real age"] == age].sample(n=fraction_nat, random_state=seed)
    df_test = df_test.append(natural_survival)

    fraction_int = int(age_prop.iloc[age, 2]*len(df.index[df.index == 1]))
    intervention_survival = df_source[df_source["Real age"] == age].sample( n=fraction_int, random_state=seed)
...
plt.figure()
ax = sns.distplot(df_test.index, label="True age")
ax = sns.distplot(age_prop_pred, color="r", label="Predicted age")
...
plt.figure()
ax = sns.distplot(df_test_intervention.index, label="True age")
ax = sns.distplot(age_prop_int_pred, color="r", label="Predicted age")
ax.set(xlim=(0, 25), xlabel="Mosquito age", ylabel="Proportion in population")
...
for age in df_source["Real age"].unique():
    natural_survival = df_source[df_source["Real age"] == age].sample(
        frac=age_prop.iloc[age, 0], random_state=seed)
    df_test = df_test.append(natural_survival)
    intervention_survival = df_source[df_source["Real age"] == age].sample(
        frac=age_prop.iloc[age, 2], random_state=seed)
    df_test_intervention = df_test_intervention.append(intervention_survival)
...
lr_ag_age_acc_distrib = pd.read_csv("./results/lr_ag_age_acc_distrib_SF_GR.csv")
lr_ag_age_acc_distrib = np.round(100 * lr_ag_age_acc_distrib)
